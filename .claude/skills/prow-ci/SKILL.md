...
# Prow CI Analysis for GCP Project Operator
...
```text
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_gcp_project_operator/<PR_NUMBER>/pull-ci-openshift-gcp-project-operator-master-lint/<BUILD_ID>
```
...
**Job History**: https://prow.ci.openshift.org/?repo=openshift%2Fgcp-project-operator
...
- **Pull request jobs**: `gs://test-platform-results/pr-logs/pull/openshift_gcp_project_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]`
- **Periodic jobs**: `gs://test-platform-results/logs/[JOB_NAME]/[JOB_ID]`

**Viewing logs via web**:
```text
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_gcp_project_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]
```
...
- `pull-ci-openshift-gcp-project-operator-master-e2e-binary-build-success` - E2E binary build verification
- `pull-ci-openshift-gcp-project-operator-master-coverage` - Code coverage analysis (with Codecov)
- `pull-ci-openshift-gcp-project-operator-master-lint` - Linting checks
- `pull-ci-openshift-gcp-project-operator-master-test` - Unit tests
- `pull-ci-openshift-gcp-project-operator-master-validate` - Validation checks
...
```text
https://prow.ci.openshift.org/?repo=openshift%2Fgcp-project-operator&type=presubmit
```
...
- Configuration: `ci-operator/config/openshift/gcp-project-operator/openshift-gcp-project-operator-master.yaml`
- Runs: lint, test, validate, coverage, e2e-binary-build
- Uses Codecov for coverage reporting (secret: `gcp-project-operator-codecov-token`)
...
gh pr checks <PR_NUMBER> | grep "pull-ci-openshift-gcp-project-operator"
...
# https://prow.ci.openshift.org/?repo=openshift%2Fgcp-project-operator

# Or use platform-specific command:
# macOS: open "https://prow.ci.openshift.org/?repo=openshift%2Fgcp-project-operator"
# Linux: xdg-open "https://prow.ci.openshift.org/?repo=openshift%2Fgcp-project-operator"
# Windows: start "https://prow.ci.openshift.org/?repo=openshift%2Fgcp-project-operator"

# View specific PR on Prow (replace <PR_NUMBER>)
# https://prow.ci.openshift.org/?repo=openshift%2Fgcp-project-operator&type=presubmit&pull=<PR_NUMBER>
```
...
- Prow jobs: `pull-ci-openshift-gcp-project-operator-master-*`
- Tekton jobs: Usually show as "Tekton" or pipeline names in PR checks
- Verify repo name format in Prow: `openshift_gcp_project_operator` (underscore, not dash)
...
- Common issues:
  ...
  - Auth failures → Secret configuration in `gcp-project-operator-tenant` namespace
- Local validation:
  ```bash
...
- Location: `ci-operator/config/openshift/gcp-project-operator/openshift-gcp-project-operator-master.yaml`
- Update process: Submit PR to openshift/release repository
- Auto-generated jobs in: `ci-operator/jobs/openshift/gcp-project-operator/`
...
- Secret: `gcp-project-operator-codecov-token` (stored in Prow)
- Generate coverage locally: `make coverage`
- Coverage runs on PRs and post-merge (`publish-coverage`)
- Dashboard: Check Codecov for gcp-project-operator

...
1. First, we need to find the AMI in the region (AWS amis have different IDs based on region). A quick way to find an official Red Hat AMI is to search for a known ami-id in a known region, find the account ID that AMI is published under, and then search for that account id under the new region.
    ...
    1. Go to the EC2 -> AMIs page and search for the ami id for the us-east-1 region from the [operator ConfigMap template](https://github.com/openshift/aws-account-operator/blob/master/hack/templates/aws.managed.openshift.io_v1alpha1_configmap.tmpl)
...
1. Add this information to the [hack/templates/aws.managed.openshift.io_v1apha1_configmap](https://github.com/openshift/aws-account-operator/blob/master/hack/templates/aws.managed.openshift.io_v1alpha1_configmap.tmpl) file.

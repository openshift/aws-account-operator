...
# Run focused test
ginkgo -focus="AccountClaim" ./controllers/accountclaim/
...
It("should create account", func() {
    // Create AccountClaim resource
    accountClaim := &v1alpha1.AccountClaim{...}
    Expect(k8sClient.Create(ctx, accountClaim)).To(Succeed())
    ...
    Expect(k8sClient.Get(ctx, accountKey, account)).To(Succeed())
    Expect(account.Status.State).To(Equal("Ready"))
})
```

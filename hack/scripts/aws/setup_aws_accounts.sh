...
assume_aws_role() {
    AWS_ASSUME_ROLE=$(aws sts assume-role --profile "$2" --role-arn arn:aws:iam::"$1":role/OrganizationAccountAccessRole --role-session-name aao-account-setup)
...
ASSUME_ROLE_POLICY_ARN=$(aws iam create-policy --policy-name STSAssumeRolePolicy${ID} --policy-document file://./setup-aws-policies/AssumeRolePolicy.json | jq '.Policy.Arn')
# Strip quotes from policy arn
ASSUME_ROLE_POLICY_ARN=$(echo ${ASSUME_ROLE_POLICY_ARN} | tr -d '"')
...
PREPARED_JSON=$(cat setup-aws-policies/JumpRoleTrustRelationship.json | jq --arg TRUST_ARN_FORMAT $TRUST_ARN_FORMAT '.Statement[0].Principal.AWS |= $TRUST_ARN_FORMAT')
# Create the Role and save its ARN, we'll use it later.
JUMP_ROLE_ARN=$(echo $PREPARED_JSON | aws iam create-role --role-name JumpRole${ID} --assume-role-policy-document file:///dev/stdin | jq '.Role.Arn')
# Attach the ARN generated by the Policy to the Jump Role we created
ATTACH_ROLE_POLICY=$(aws iam attach-role-policy --role-name JumpRole${ID} --policy-arn "${ASSUME_ROLE_POLICY_ARN}")
...
# Stript quotes from jump ARN
JUMP_ROLE_ARN=$(echo ${JUMP_ROLE_ARN} | tr -d '"')
# Update AccessRoleTrustRelationship with the following Jump Role ARN
PREPARED_JSON=$(cat setup-aws-policies/AccessRoleTrustRelationship.json | jq --arg JUMP_ROLE_ARN ${JUMP_ROLE_ARN} '.Statement[0].Principal.AWS |= $JUMP_ROLE_ARN')
...
max_retries=5
i=1
while ! STS_ROLE_ARN=$(echo $PREPARED_JSON | aws iam create-role --role-name AccessRole${ID} --assume-role-policy-document file:///dev/stdin)
do
  if [[ $i > $max_retries ]]
  then
    echo "Could not create jump role"
    exit 1
  fi
  ((j=2**i))
  echo "Jump Role Arn not ready yet."
  echo "sleeping $j"
  sleep $j
  ((i=i+1))
done

STS_ROLE_ARN=$(echo ${STS_ROLE_ARN} | jq '.Role.Arn')

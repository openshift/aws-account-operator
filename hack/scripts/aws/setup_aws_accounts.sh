...
assume_aws_role() {
    local account_1=$1
    local account_2=$2
    AWS_ASSUME_ROLE=$(aws sts assume-role --profile "$account_2" --role-arn arn:aws:iam::"$account_1":role/OrganizationAccountAccessRole --role-session-name aao-account-setup)
...
ASSUME_ROLE_POLICY_ARN=$(aws iam create-policy --policy-name STSAssumeRolePolicy --policy-document file://./setup-aws-policies/AssumeRolePolicy.json | jq '.Policy.Arn')
...
PREPARED_JSON=$(< setup-aws-policies/JumpRoleTrustRelationship.json jq --arg TRUST_ARN_FORMAT "$TRUST_ARN_FORMAT" '.Statement[0].Principal.AWS |= $TRUST_ARN_FORMAT')
# Create the Role and save its ARN, we'll use it later.
JUMP_ROLE_ARN=$(echo "$PREPARED_JSON" | aws iam create-role --role-name JumpRole --assume-role-policy-document file:///dev/stdin | jq '.Role.Arn')
# Attach the ARN generated by the Policy to the Jump Role we created
aws iam attach-role-policy --role-name JumpRole --policy-arn "${ASSUME_ROLE_POLICY_ARN}"
...
PREPARED_JSON=$(< setup-aws-policies/AccessRoleTrustRelationship.json jq --arg JUMP_ROLE_ARN "${JUMP_ROLE_ARN}" '.Statement[0].Principal.AWS |= $JUMP_ROLE_ARN')
...
max_retries=5
i=1
while ! STS_ROLE_ARN=$(echo "$PREPARED_JSON" | aws iam create-role --role-name AccessRole --assume-role-policy-document file:///dev/stdin)
do
  if [[ $i > $max_retries ]]
  then

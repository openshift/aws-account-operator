...
oc process -p AWS_IAM_ARN="${AWS_IAM_ARN}" -p IAM_USER_SECRET="${IAM_USER_SECRET}" -p AWS_FEDERATED_ROLE_NAME="${AWS_FEDERATED_ROLE_NAME}" -p NAMESPACE="${NAMESPACE}" -p FED_USER="${FED_USER}" -f hack/templates/aws_v1alpha1_awsfederatedaccountaccess_cr.tmpl | oc apply -f -

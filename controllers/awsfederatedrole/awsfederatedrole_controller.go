...
func (r *AWSFederatedRoleReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	if instance.DeletionTimestamp != nil {
		...
		if utils.Contains(instance.GetFinalizers(), utils.Finalizer) {
			...
			err = r.finalizeFederateRole(ctx, reqLogger, instance)
			if err != nil {
				return reconcile.Result{}, err
			}

			reqLogger.Info("Removing Finalizer")
			err = r.removeFinalizer(ctx, reqLogger, instance, utils.Finalizer)
			if err != nil {
				return reconcile.Result{}, err
			}
		}
	...
	createOutput, err := awsClient.CreatePolicy(ctx, &iam.CreatePolicyInput{
	...
	_, err = awsClient.DeletePolicy(ctx, &iam.DeletePolicyInput{PolicyArn: createOutput.Policy.Arn})
	if err != nil {
	...
	managedPolicies, err := getAllPolicies(ctx, awsClient)
	if err != nil {
	...
	if err := annotateAccountAccesses(ctx, r.Client, instance.Name); err != nil {
...
func annotateAccountAccesses(ctx context.Context, kubeClient client.Client, roleName string) error {
	accountAccesses := &awsv1alpha1.AWSFederatedAccountAccessList{}
	err := kubeClient.List(ctx, accountAccesses, client.MatchingLabels{awsv1alpha1.FederatedRoleNameLabel: roleName})
	if err != nil {
		return err
	}
	for i := range accountAccesses.Items {
		...
		err = kubeClient.Update(ctx, &accountAccesses.Items[i])
		if err != nil {
			return err
		}
	}
...
func getAllPolicies(ctx context.Context, awsClient awsclient.Client) ([]iamtypes.Policy, error) {
	...
	for {
		output, err := awsClient.ListPolicies(ctx, input)
		if err != nil {

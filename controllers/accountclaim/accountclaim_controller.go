...
func (r *AccountClaimReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	if accountClaim.DeletionTimestamp != nil {
		if accountClaim.Spec.FleetManagerConfig.TrustedARN != "" {
			if r.checkIAMSecretExists(accountClaim.Spec.AwsCredentialSecret.Name, accountClaim.Spec.AwsCredentialSecret.Namespace) {
				...
				reqLogger.Info("successfully deleted IAM secret", "accountclaim", accountClaim.Name)
			}
			...
			reqLogger.Info("successfully got claimed account", "accountclaim", accountClaim.Name)
			if currentAcctInstance != nil && !currentAcctInstance.IsBYOC() {
				...
				reqLogger.Info("successfully cleaned up IAM role and policies", "accountclaim", accountClaim.Name)
			}
		}
	...
	if accountClaim.Spec.AccountLink == "" {
		...
		reqLogger.Info("successfully got unclaimed account", "accountclaim", accountClaim.Name)
	} else {
	...
	if unclaimedAccount.Spec.ClaimLink == "" {
		...
		reqLogger.Info("successfully updated claimLink", "accountclaim", accountClaim.Name)
	}

	// Set awsAccountClaim.Spec.AccountLink
	if accountClaim.Spec.AccountLink == "" {
		setAccountLinkOnAccountClaim(reqLogger, unclaimedAccount, accountClaim)
		reqLogger.Info("successfully set AccountLink", "accountclaim", accountClaim.Name)
	...
	if !accountClaim.Spec.ManualSTSMode {
		...
		reqLogger.Info("successfully set the support role ARN", "accountclaim", accountClaim.Name)
		if err != nil {
			return reconcile.Result{}, err
		}
	}
	...
	if accountClaim.Spec.AccountOU == "" || accountClaim.Spec.AccountOU == "ROOT" {
		...
		reqLogger.Info("successfully moved account to OU", "accountclaimName", accountClaim.Name, "account", unclaimedAccount.Name)
	}
	...
	if accountClaim.Spec.FleetManagerConfig.TrustedARN != "" && (accountClaim.Spec.AccountPool != "" && accountClaim.Spec.AccountPool != "default") {
		if fleetManagerClaimEnabled {
			...
			reqLogger.Info("successfully created role and secret for fleet manager accountclaim", "accountclaim", accountClaim.Name)
		} else {
		...
		if !r.checkIAMSecretExists(accountClaim.Spec.AwsCredentialSecret.Name, accountClaim.Spec.AwsCredentialSecret.Namespace) {
			...
			reqLogger.Info("successfully created IAM secret", "accountclaim", accountClaim.Name)
		}
	}
	...
	if accountClaim.Status.State != awsv1alpha1.ClaimStatusReady && accountClaim.Spec.AccountLink != "" {
		...
		reqLogger.Info("successfully updated accountclaim status to Ready", "accountclaim", accountClaim.Name)
		return reconcile.Result{}, r.statusUpdate(reqLogger, accountClaim)
	}
...
func (r *AccountClaimReconciler) handleBYOCAccountClaim(reqLogger logr.Logger, accountClaim *awsv1alpha1.AccountClaim) (reconcile.Result, error) {
	...
	if accountClaim.Spec.AccountLink == "" {
		...
		reqLogger.Info("successfully validated account linked to accountclaim ", "accountclaim", accountClaim.Name)
		...
		reqLogger.Info("successfully created account for BYOC claim", "accountclaim", accountClaim.Name, "account", accountClaim.Spec.AccountLink)
	...
	if byocAccount.IsReady() && accountClaim.Status.State != awsv1alpha1.ClaimStatusReady {
		...
		reqLogger.Info(fmt.Sprintf("%s is Ready", byocAccount.Name), "accountclaim", accountClaim.Name, "Account Status", byocAccount.Status.State)
		// Update the status on AccountClaim
		return reconcile.Result{}, r.statusUpdate(reqLogger, accountClaim)
	}
	...
	if !accountClaim.Spec.ManualSTSMode {
		...
		reqLogger.Info("successfully set support role ARN", "accountclaim", accountClaim.Name)

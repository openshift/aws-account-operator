...
// Test the accountNeedsCredentialsRotated function
func TestAccountNeedsCredentialsRotated(t *testing.T) {
	tests := []struct {
		name     string
		expected bool
		acct     *testAccountBuilder
		...
		{
			name:     "Account has 'RotateCredentials' set to true",
			acct:     newTestAccountBuilder().RotateCredentials(true),
			expected: true,
		},
		{
			name:     "Account does not 'RotateCredentials' set to true",
			acct:     newTestAccountBuilder(),
			expected: false,
		},
	}
	for _, test := range tests {
		t.Run(
			test.name,
			func(t *testing.T) {
				result := accountNeedsCredentialsRotated(&test.acct.acct)
				if result != test.expected {
					t.Error(
						"for account:", test.acct,
						"expected", test.expected,
...
// Test the accountNeedsConsoleCredentialsRotated function
func TestAccountNeedsConsoleCredentialsRotated(t *testing.T) {
	tests := []struct {
		name     string
		expected bool
		acct     *testAccountBuilder
		...
		{
			name:     "Account has 'RotateConsoleCredentials' set to true",
			acct:     newTestAccountBuilder().RotateConsoleCredentials(true),
			expected: true,
		},
		{
			name:     "Account does not 'RotateConsoleCredentials' set to true",
			acct:     newTestAccountBuilder(),
			expected: false,
		},
	}
	for _, test := range tests {
		t.Run(
			test.name,
			func(t *testing.T) {
				result := accountNeedsConsoleCredentialsRotated(&test.acct.acct)
				if result != test.expected {
					t.Error(
						"for account:", test.acct,
						"expected", test.expected,

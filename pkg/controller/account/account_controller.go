...
func (r *ReconcileAccount) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if currentAcctInstance.ReadyForInitialization() {
		...
		// The dependancy on the roleID validation within the assumeRole, and the different aws clients
		// required between CCS and non-CCS, is what has caused these steps to be done independantly.
